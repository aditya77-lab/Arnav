...
def initialize_gemini():
    ...
    model = genai.GenerativeModel(
        ...
        system_instruction=(
            "You are a professional fact-checker and news analyst. Your task is to critically analyze news content "
            "for authenticity and potential misinformation. Follow these strict guidelines:\n\n"
            "1. Source Analysis:\n"
            "   - Check if the source is reputable and well-known\n"
            "   - Verify if the source has a history of accurate reporting\n"
            "   - Look for official sources and citations\n\n"
            "2. Content Analysis:\n"
            "   - Check for factual claims and verify if they can be supported\n"
            "   - Look for emotional manipulation or sensationalism\n"
            "   - Identify any logical fallacies or inconsistencies\n"
            "   - Check for proper attribution of quotes and information\n\n"
            "3. Language Analysis:\n"
            "   - Look for biased or inflammatory language\n"
            "   - Check for clickbait-style headlines\n"
            "   - Identify any attempts to manipulate emotions\n\n"
            "4. Evidence Evaluation:\n"
            "   - Check for supporting evidence for major claims\n"
            "   - Look for expert opinions or official statements\n"
            "   - Verify if statistics or data are properly cited\n\n"
            "Provide your analysis in this exact format:\n"
            "1. Final Verdict: [REAL/FAKE/UNVERIFIED]\n"
            "2. Key Points:\n"
            "   - [List 3-5 key points that influenced your decision]\n"
            "3. Explanation:\n"
            "   - [Provide a detailed explanation of your analysis]\n"
            "4. Recommendations:\n"
            "   - [Suggest how to verify or fact-check specific claims]\n\n"
            "Important: Be conservative in your assessment. If you cannot verify a claim, mark it as UNVERIFIED "
            "rather than making assumptions. Focus on factual analysis rather than opinion."
        ),
    )
...
def get_gemini_response(chat_session, description, retries=3, delay=10):
    try:
        ...
        enhanced_prompt = f"""
        Please analyze this news content with extreme care and attention to detail:

        CONTENT TO ANALYZE:
        {description}

        Follow these steps in your analysis:
        1. First, identify all factual claims in the content
        2. Then, evaluate each claim for supporting evidence
        3. Check for source credibility and attribution
        4. Look for any red flags or inconsistencies
        5. Consider the overall context and presentation

        Provide your analysis in the exact format specified in the system instructions.
        Be thorough but concise. If you cannot verify a claim, mark it as UNVERIFIED.
        """
        ...
        if "429" in error_str or "quota" in error_str.lower():
            st.warning(f"Rate limit/quota exceeded. Waiting for {delay} seconds...")
            time.sleep(delay)
            if retries > 0:
                return get_gemini_response(chat_session, description, retries - 1, delay * 2)
            st.error("Max retries reached. Please try again later or upgrade your API quota.")
            return None
...
def main_page():
    ...
    for i, (content, result, _, _, date) in enumerate(history):
